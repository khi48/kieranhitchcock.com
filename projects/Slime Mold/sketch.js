...
function preload(){
  ...
  //font = loadFont('assets/dancing-script-ot/DancingScript-Regular.otf');
}
...
function mousePressed(){
  if (last_draw){
    ...
    //alive_branches.insert([mouseX+branch_length*dir[0], mouseY+branch_length*dir[1], mouseX, mouseY, dir[0], dir[1]]);
...
function updateAliveBraches(node) {
  ...
  if (node.leaf) { // checking if it is the end array of the rbush tree
    ...
    for (var i = 0; i < node.children.length; i++) {
      ...
      var leaves_too_close = knn(leaves, branch[0], branch[1], null, null, min_distance); // return all nearest items around point [40, 40]
    ...
    for (var i = 0; i < node.children.length; i++) {
      ...
      if (closest_branch){
        ...
        if (closest_branch[6]>=branch_length){
          ...
          //print([new_branch_x, new_branch_y, closest_branch[0], closest_branch[1], dir_x, dir_y, grow_length, 1])
...
function draw() {
  ...
  if (leaves.data.children.length > 0){
    ...
    // strokeWeight(2);
    // fill(255);
    // line(200+max_distance/2,200-max_distance/2, 200+max_distance/2,200+max_distance/2)
  }

  // stroke(255);
  // strokeWeight(2);
  // fill(255);
  // line(200,200, 200 + max_distance,200)
